...
pub trait UpdateReadmeFiles {
    ...
    async fn update_readme_files(&self) -> Result<(), Self::Error>;
}
...
impl UpdateReadmeFiles for CrateHandle {
    ...
    async fn update_readme_files(&self) -> Result<(), Self::Error> {
        ...
        let requests = vec![AiReadmeWriterRequest::async_try_from::<PathBuf,CrateHandle>(self).await?];

        execute_ai_readme_writer_requests(&requests)
            .await
            .map_err(|_| ReadmeWriteError::AiReadmeWriterError)?;
      ...
      + Sync + Send + 'static,
{
    ...
    async fn update_readme_files(&self) -> Result<(), Self::Error> {
        ...
        for item in self.crates().iter() {
            let request = AiReadmeWriterRequest::async_try_from::<P,H>(item).await?;
